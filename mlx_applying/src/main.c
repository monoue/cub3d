...
{
	if (argc < 2)
		return (exit_failure_with_error_message("You need argument(s).\n"));
	if (ft_strlen(argv[1] < 5) || ft_strncmp(&argv[1][ft_strlen(argv[1] - 4)], ".cub", 4) != 0)
	// if (!ft_strnstr(argv[1], ".cub", ft_strlen(argv[1])))
		return (exit_failure_with_error_message(".cub file must be specified.\n"));
	if (argc > 2)
	{
		if (ft_strncmp(argv[2], "--save", ft_strlen(argv[2])) != 0)
			return (exit_failure_with_error_message("Invalid option. Option: --save\n"));
		save_picture(argv[1]);
	}
	else
		init_game(argv[1]);
